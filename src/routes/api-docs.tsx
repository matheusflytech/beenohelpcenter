...
import { ChevronRight, Copy, Check, Search, Download } from "lucide-react";
import {
  endpointGroups,
  totalEndpoints,
  FILTER_OPERATORS,
...
// Builds a standard Postman Collection v2.1 from our own endpoint data,
// so anyone can import it directly into Postman, Insomnia, Bruno, etc.
function buildPostmanCollection() {
  return {
    info: {
      name: "Beeno API",
      description: `Coleção com os ${totalEndpoints} endpoints da API do Beeno CRM, organizados por recurso.`,
      schema: "https://schema.getpostman.com/json/collection/v2.1.0/collection.json",
    },
    variable: [
      { key: "clientId", value: "", description: "Identificador da sua conta no Beeno" },
      { key: "apiKey", value: "", description: "Sua API Key (ELOZ-APIKEY)" },
    ],
    item: endpointGroups.map((group) => ({
      name: group.name,
      description: group.desc ?? "",
      item: group.endpoints.map((ep) => {
        const path = ep.path.replace(/^\/api\/v1/, "");
        const request: Record<string, unknown> = {
          method: ep.method,
          header: [
            { key: "ELOZ-APIKEY", value: "{{apiKey}}", type: "text" },
            ...(ep.body ? [{ key: "Content-Type", value: "application/json", type: "text" }] : []),
          ],
          url: {
            raw: `https://app.beeno.ai/{{clientId}}/api/v1${path}`,
            protocol: "https",
            host: ["app", "beeno", "ai"],
            path: `{{clientId}}/api/v1${path}`.split("/").filter(Boolean),
          },
          description: ep.desc,
        };
        if (ep.body) {
          request.body = { mode: "raw", raw: ep.body, options: { raw: { language: "json" } } };
        }
        return { name: ep.desc, request, response: [] };
      }),
    })),
  };
}
...
function downloadCollection() {
  ...
  a.href = url;
  a.download = "beeno-api-collection.json";
      ...
      <aside className="w-[280px] shrink-0 bg-[#1b1c1e] flex flex-col border-r border-black/40 sticky top-16 h-[calc(100vh-4rem)]">
        <div className="p-5 pb-4 border-b border-white/10 shrink-0">
          ...
          <button
            ...
            className="mt-3 w-full flex items-center justify-center gap-2 rounded-md bg-emerald-500 hover:bg-emerald-600 text-white text-[13px] font-semibold py-2 transition"
          >
